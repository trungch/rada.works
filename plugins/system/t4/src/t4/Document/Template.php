...
class Template
{
    ...
    protected function renderOpenGraph(){
        ...
        if(!$this->doc->params->get('system_opengraph',"")) return;
        $options = $this->app->input->get('option','');
        if(in_array($options,array('com_content', 'com_t4pagebuilder',"com_contact"))) return;
        ...
        if($actual_link == $active_link && $params->get('og_title','')){
            $desc = trim(strip_tags(\JHTML::_('string.truncate', strip_tags($params->get('og_desc',$configs->get('MetaDesc'))), 110)));
            ...
            if($images->url) Metadata::renderTag('og:image',Metadata::fullImageURL($images->url),2);
            ...
            if($images->url) Metadata::renderTag('twitter:image',Metadata::fullImageURL($images->url),2);
        }
    }
    ...
    {   
        ...
        return \JLayoutHelper::render($layout, $this, T4PATH . '/html/layouts');
    }
    ...
    public function afterRender()
    {
        ...
        if(defined("T4_BS5")){
            $buffer = str_replace(array('data-toggle','data-title','data-dismiss','data-trigger', 'data-target'), array('data-bs-toggle','data-bs-title','data-bs-dismiss','data-bs-trigger', 'data-bs-target'), $buffer);
        }
        $app->setBody($buffer);
    }
    ...
    public function compileHead()
    {
        ...
        if(!(\T4\T4::inEdit() && $t4EditLayout)){
            ...
            if(!file_exists(JPATH_ROOT.$template_style_custom_css) && Layout::isSubpage() && !\T4\T4::inEdit()){
                $this->renderCustomCss();
            }
