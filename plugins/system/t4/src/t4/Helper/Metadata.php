	...
	public static function renderOpenGraph($context, $item, $params)
	{
		...
		if(in_array('com_content',explode('.', $context))){
			if($views == 'category' && self::$metaNr == 0){
				...
				if(!$og_desc){
					$og_desc = HTMLHelper::_('string.truncate', trim(strip_tags($item->metadesc ?: (!empty($item->description) ? $item->description : ""))), 110);
				}
				if(!$og_image){
				...
				if(!$og_desc){
					$og_desc = HTMLHelper::_('string.truncate', trim(strip_tags($item->metadesc ?: ($item->introtext ?: $item->fulltext))), 110);
				}
				if(!$og_image){
			...
			if(!$og_desc){
				$og_desc = HTMLHelper::_('string.truncate', trim(strip_tags($item->metadesc ?: ($item->misc ?: ''))), 110);
			}
			if(!$og_image){
				$og_image = $item->image;
			}
