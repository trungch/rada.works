		...
		if ($this->params->get('show_autosuggest', 1))
			{
				...
				$wam->useScript('jquery.autocomplete');
				$script .= "
				...
				<input type="text" name="q" id="q" class="js-finder-search-query form-control" value="<?php echo $this->escape($this->query->input); ?>">
					<span class="input-group-append">
						...
						<?php if ($this->params->get('show_advanced', 1)) : ?>
							...
							<?php else: ?>
								<a href="#advancedSearch"<?php echo \T4\Helper\T4Bootstrap::getAttrs(array('toggle'=>'collapse','target'=>'#advancedSearch'));?>class="btn" aria-expanded="<?php echo ($this->params->get('expand_advanced', 0) ? 'true' : 'false'); ?>">
